...
export default function AIAssistant({ user, isPremium, onOpenAuth, onTriggerUpgrade }: AIAssistantProps) {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    // Add current prompt
    let currentPart: any = { text: userMsgText };
    if (attachedFileBase64 && attachedFile) {
      currentPart = {
        parts: [
          ...
          { text: userMsgText || 'Explain this file' }
        ]
      };
    } else {
      currentPart = {
    ...
    try {
      ...
      const errMsg: Message = {
        ...
        sender: 'ai',
        text: `Error connecting to ImgDocs AI service: ${err.message || 'Please verify your network connection and try again.'}`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      setAnalyzeResult(`Analysis Error: ${err.message || 'Failed to analyze this document.'}`);
    } finally {
                ...
                <div className="p-2.5 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-bold font-mono text-[10px] text-indigo-600 dark:text-indigo-400">
                      {isPremium ? "gemini-3.5-flash" : "gemini-3.1-flash-lite"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
